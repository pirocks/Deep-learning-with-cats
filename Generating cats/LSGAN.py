...
parser.add_argument('--lr_D', type=float, default=.0001,
					help='Discriminator learning rate')  # 1/4 of DCGAN paper original value
...
parser.add_argument('--a', type=bool, default=0,
					help='In Discriminator loss: 1/2*E[(D(x_real)-b)^2]+1/2*E[(D(x_fake-a)^2]')  # LSGAN paper original value
parser.add_argument('--b', type=bool, default=1,
					help='In Discriminator loss: 1/2*E[(D(x_real)-b)^2]+1/2*E[(D(x_fake-a)^2]')  # LSGAN paper original value
...
parser.add_argument('--output_folder', default='/home/alexia/Output/LSGAN', help='output folder')
...
parser.add_argument('--n_workers', type=int, default=2,
					help='Number of subprocess to use to load the data. Use at least 2 or the number of cpu cores - 1.')
parser.add_argument('--weight_decay', type=float, default=0,
					help='L2 regularization weight. Greatly helps convergence but leads to artifacts in images, not recommended.')
parser.add_argument('--gen_extra_images', type=int, default=0,
					help='Every epoch, generate additional images with "batch_size" random fake cats.')
	...
	for i, data_batch in enumerate(dataset, 0):
		...
		if i % 50 == 0:
			...
			print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f time:%.4f' % (
				epoch, param.n_epoch, i, len(dataset), errD.data[0], errG.data[0], end - start))
			print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f time:%.4f' % (
				epoch, param.n_epoch, i, len(dataset), errD.data[0], errG.data[0], end - start), file=log_output)
	# Save every epoch
	if epoch % 25 == 0:
